...
const CoursePlayer: React.FC<CoursePlayerProps> = ({ course, onBack }) => {
  const [selectedLesson, setSelectedLesson] = useState<Lesson | null>(
    course.topics && course.topics.length > 0 ? null : course.lessons[0]
  );
  const [selectedTopic, setSelectedTopic] = useState<Topic | null>(null);
  const [viewMode, setViewMode] = useState<'topics' | 'lessons'>(
    course.topics && course.topics.length > 0 ? 'topics' : 'lessons'
  );
  ...
  if (viewMode === 'topics' && course.topics && course.topics.length > 0) {
    const activeTopics = course.topics.filter(topic => topic.isActive).sort((a, b) => a.order - b.order);
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center justify-between text-sm text-gray-400">
                    ...
                    <span className="text-blue-400">Assistir Tópico</span>
                  </div>
                </div>
              </div>
            ))}
      ...
      <div className="flex h-[calc(100vh-80px)]">
        ...
        <div className="w-80 bg-gray-800 border-l border-gray-700 overflow-y-auto">
          <div className="p-4 border-b border-gray-700">
            <h2 className="text-lg font-semibold mb-2">Aulas do Curso</h2>
            <p className="text-sm text-gray-400">{course.lessons.length} aulas</p>
          </div>
